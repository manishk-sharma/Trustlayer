    ...
    <div className="animate-fade-in-up space-y-5 pb-6">
      ...
      <div className="text-center space-y-1">
        ...
        <p className="text-xs text-text-muted">
          {transactions.length} transaction{transactions.length !== 1 ? "s" : ""} this session
        </p>
      </div>
      ...
      <div className="flex gap-2">
        {[
          {
            label: "Low",
            count: transactions.filter((t) => t.risk_level === "LOW").length,
            color: "bg-risk-low",
            textColor: "text-risk-low",
          },
          {
            label: "Medium",
            count: transactions.filter((t) => t.risk_level === "MEDIUM").length,
            color: "bg-risk-medium",
            textColor: "text-risk-medium",
          },
          {
            label: "High",
            count: transactions.filter((t) => t.risk_level === "HIGH").length,
            color: "bg-risk-high",
          ...
          <div
            key={stat.label}
            className="flex-1 glass-card rounded-xl p-2.5 text-center border border-navy-700"
          >
            <p className={`text-lg font-bold tabular-nums ${stat.textColor}`}>
              {stat.count}
            </p>
      ...
      <div className="space-y-2">
        <form onSubmit={handleSearchSubmit} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search recipient, note, risk level..."
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-3.5 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-secondary cursor-pointer flex items-center justify-center p-0.5"
              >
          ...
          <button
            ...
            type="submit"
            className="px-4 py-2.5 rounded-xl text-xs font-semibold bg-electric text-white hover:bg-electric-dark transition-colors cursor-pointer"
          >
            Search
          </button>
        </form>
        ...
        {searchHistory.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 text-xs pt-1">
            <span className="text-text-muted font-medium">Recent:</span>
            {searchHistory.map((query, index) => (
              <div
                key={index}
                onClick={() => setSearchQuery(query)}
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-navy-800 border border-navy-700 text-text-secondary hover:text-text-primary hover:border-electric/30 cursor-pointer transition-colors"
              >
                <span>{query}</span>
                <button
                  type="button"
                  onClick={(e) => removeFromHistory(e, query)}
                  className="text-text-muted hover:text-risk-high cursor-pointer flex items-center justify-center p-0.5 rounded-full hover:bg-gray-200 transition-colors"
                >
            ...
            <button
              ...
              className="text-text-muted hover:text-text-secondary hover:underline cursor-pointer ml-auto text-[11px]"
            >
              Clear all
            </button>
          </div>
              ...
              <div className="animate-fade-in-up mt-1 ml-2 mr-2">
                <div className="bg-gray-50 rounded-xl p-4 space-y-3 border border-gray-200">
                  ...
                  {txn.fraud_type && txn.fraud_type !== "None" && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-text-muted">Fraud Type</span>
                  ...
                  <div className="flex items-start gap-2 bg-blue-50 rounded-lg p-2.5">
                    ...
                    <p className="text-[11px] text-electric-light leading-relaxed">
                      {txn.recommendation}
                    </p>
                  </div>
                </div>
      ...
      {transactions.length > 0 && filteredTransactions.length === 0 && (
        <div className="text-center py-12 glass-card rounded-xl p-6 border border-navy-700">
          ...
          <p className="text-sm text-text-primary font-semibold">No matches found</p>
          <p className="text-xs text-text-muted mt-1">
            We couldn't find any transaction matching "{searchQuery}"
          </p>
          <button
            onClick={() => setSearchQuery("")}
            className="mt-4 px-4 py-2 rounded-xl text-xs font-semibold bg-navy-800 text-text-secondary hover:bg-navy-700 transition-colors cursor-pointer border border-navy-700 flex items-center gap-1 mx-auto"
          >
